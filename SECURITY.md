...
- Account records, settings, failure reports, mobile pairing state, and sandbox artifacts are local data and are excluded from Git.
...
- `learning` paired devices are restricted at the gateway to account-self, Personal Study, Forge play, and own-progress routes. `full_role` devices still rely on normal SAD RBAC after the device gate.
- Revoking a paired device invalidates its device token server-side.
- The service worker caches only static shell assets and explicitly excludes `/v1/*` and `/mobile/*` traffic.
