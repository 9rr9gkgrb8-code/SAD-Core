...
class FakeWorkspaceStore:
    def list(self):
        return [{"workspace_id": "11111111-1111-1111-1111-111111111111", "state": "tests_passed"}]

    def get(self, workspace_id):
        return {"workspace_id": workspace_id, "state": "tests_passed", "diff": "diff"}
    ...
    def execute(self, workspace_id):
        return {"workspace_id": workspace_id, "state": "tests_passed", "tests": {"passed": True}}
...
class DeveloperWorkspaceApiTests(unittest.TestCase):
    def setUp(self):
        ...
        for username, role in (("developer", "developer"), ("reviewer", "reviewer"), ("viewer", "viewer"), ("student", "student")):
            self.auth.create_account(username, f"Strong{username.title()}123", role, self.owner)
        self.tokens = {name: self.auth.login(name, f"Strong{name.title()}123") for name in ("developer", "reviewer", "viewer", "student")}
    ...
    def test_developer_can_plan_create_and_execute_but_cannot_apply(self):
